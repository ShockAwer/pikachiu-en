...
#	print "<!--#echo banner=\"\"-->";
	...
	print "<font size=-1><b><a href=\"mailto:$mailadd\">連絡先</a> " if ($mailadd ne '');
	print "<a href=\"$supporturl\">サポート掲示板</a>" if ($supporturl ne '');
	...
	print "\n<a href=\"http://dauso.virtualave.net/cgi-bin/bbs.cgi\">あやしい＠ぴかちう</a>｜\n";
	print "<a href=\"http://www.org1.com/~osamu/pbbs.cgi\">ぴかちゅう＠メイソ\<font size=-1><sup>TM</sup></font></a><p>\n";
	print "<font color=white>最近の過去ログは</font><a href=\"./$logpath\">ここ</a></td></tr></table>\n";
	...
	$e = $p_end + 1;
	$linecount++;
	if ($end < 0) { print"<hr>記事はありません"; }
	else { print "<hr>新着順 $s～$e（記録件数$linecount 最大記録件数 $max）</pre><p>\n"; }
	...
	print "<h4 align=right><a href=\"http://www.geocities.com/Tokyo/Dojo/5886/cgi/index.html\">SunechamaBBS</a> Ver2.30 ぴかちゅ仕様＋ShibaCounter+1.943</h4>\n";
	print "</body></html>\n";
	exit;
}
...
sub confirm {
	...
	print "Content-type: text/html\n\n";
	print "<html><head><title>表\示確認</title></head>\n";
	print "$body\n";
	print "<pre>";
	print "あンたが入力した文章\n";
	print "----------------------------------------\n";
...
sub printform {
	...
	print "　（） ～　○ ＼/　／\n";
	print "</pre></td></tr>\n";
	print "<tr><td><input type=checkbox name=confirm value=checked checked>表\示確認</td>\n";
	...
	print "</tr></table>\n";
	print "半角24文字（全角で12文字）×７行までです。それ以上は切り捨てられます。<p>\n";
	print "表\示件数\n";
	print "<input type=text name=\"def\" size=3 value=\"$def\" maxlength=3>\n";
	print "BGカラー\n";
	print "<input type=text name=\"bgcolor\" size=6 value=\"$bgc\">\n";
	...
	print "固定ピッチフォントを正しく設定してもずれる場合は、↑の<b>font size</b>をいろいろ変えて見てください<p>\n";
	print "<input type=submit value=\"投稿／リロード\"><input type=reset value=\"消す\"><p>\n";
	print "</form>\n";
...
sub regist {
	...
	if (-z $filedate) {
#		print LOG "<html><title>$title過去ログ $month月$mday日</title>\n";
		print LOG "<html><title>$title過去ログ $month月</title>\n";
		print LOG "$body\n";
#		print LOG "<h2 align=center>$title過去ログ $month月$mday日</h2>\n";
		print LOG "<h2 align=center>$title過去ログ $month月</h2>\n";
...
sub counter {
	...
	print "（今日：$today 昨日：$yesterday ｜ Win：$win Mac：$mac Other：$other_os ｜ IE：$ie NN他：$nn ）<br>";
}
